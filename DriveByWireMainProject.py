...
def Kar_tech_encoder(position):
    ...
    # Removing '0x' from the start of the hex
...
# Get ready to print
textPrint = TextPrint()
message_zero_brake = Message(arbitration_id=16711681, data = [int('7E', 16), int('02', 16), int('12', 16), int('34', 16), int('56', 16), int('AB', 16), int('CD', 16), int('EF', 16)]) # set brake to 0
message_zero_accelerator = Message(arbitration_id=16711682, data = [int('7E', 16), int('02', 16), int('12', 16), int('34', 16), int('56', 16), int('AB', 16), int('CD', 16), int('EF', 16)]) # set accel to 0
bus.send(message_zero_brake)
time.sleep(0.001)
bus.send(message_zero_accelerator)
...
while done == False:
    ...
    steer_pin = 21 # steering stop hum
    #steer_pin = 20 # steering stop hum

    GPIO.setmode(GPIO.BCM)

    GPIO.setup(steer_pin,GPIO.OUT) # MUST BE HIGH TO NOT EPAS FAIL
    GPIO.output(steer_pin,GPIO.LOW)
    ...
    textPrint.printtext(screen, "Number of joysticks: {}".format(joystick_count)) # should only be one
    textPrint.indent()
    ...
    # For each joystick:
    for i in range(joystick_count):  # REFACTOR. should only be one controller ever. MAKE ENUMERATE
        ...
        # for each axis
        for i in range(axes): # MAKE ENUMERATE
        ...
        for i in range(buttons): # MAKE ENUMERATE
            button = joystick.get_button(i)
            textPrint.printtext(screen, "Button {:>2} value: {}".format(i, button))  # draw button number and value
        textPrint.unindent()
        button11 = joystick.get_button(11) # LOGI twist clock(1) and anti (-1)  # Triggers, Left (1) Right (-1)

        ################ ---------------- Joystick to Can Message --------------------- ######################
        axis0 = joystick.get_axis(0) # LOGI left (1) and right (-1)        # Left knob, left (-1) and right (1)
        axis1 = joystick.get_axis(1) # LOGI front(-1) and back (1)         # Left knob, up (-1) and down (1)
        axis2 = joystick.get_axis(2) # LOGI twist clock(1) and anti (-1)  # Triggers, Left (1) Right (-1)
        button10 = joystick.get_button(10) # LOGI twist clock(1) and anti (-1)  # Triggers, Left (1) Right (-1)
        button11 = joystick.get_button(11) # LOGI twist clock(1) and anti (-1)  # Triggers, Left (1) Right (-1)
        ...
        #actuator_position = 1.5 * abs(axis1) + 1.5

        actuator_position = axis1 * 3
        speed = 1  # speed mode
        
        if manualMode == False:
            print('autonomous mode on')
            if button10 == 1:
               GPIO.output(steer_pin,GPIO.HIGH)
               time.sleep(0.5)
               GPIO.output(steer_pin,GPIO.LOW)
               bus.shutdown()
               done = True
            ...
            # TODO: recommend adding on a limit multiplier/divider of axis0 so we can expand or contract the max/min turn amoun



            multiplier = 1.5  # is used to alter the position to send. i.e. joystick full right with multiplier 5, position = 5 rev
            ...
            data_kar = Kar_tech_encoder(abs(actuator_position))
            message_send_brake = Message(arbitration_id=16711681, data = Kar_tech_encoder(1.5)) # set brake to 1.5 inch offset
            message_send_accelerator = Message(arbitration_id=16711682, data = Kar_tech_encoder(0)) # set accel to 0

            # Encoding the position
            if actuator_position < 0:
                message_send_accelerator = Message(arbitration_id=16711682, data = data_kar)
                
            if actuator_position > 1.5: # if greater than the offset
                message_send_brake = Message(arbitration_id=16711681, data = data_kar)
            
            if manualMode == False and done == False:
                bus.send(message_send_accelerator)
                time.sleep(0.001)
                bus.send(message_send_brake)
                time.sleep(0.001)
